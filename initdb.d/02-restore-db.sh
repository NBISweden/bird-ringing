...
#
# It is assumed that the application usernames and passwords have not
# changed since the dump was created.
...
if [ -s "$db_dump_file" ]; then
	...
	pg_restore --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" "$db_dump_file"
fi
